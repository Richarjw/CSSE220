...
public class Recursion {
	...
	public static int findLastWin(String input) {
		if(input.length()==0)
			return 0;
		if(input.charAt(input.length()-1)=='W')
			return input.length();
		return findLastWin(input.substring(0,input.length()-1));
	}
	...
	public static boolean hasHiddenWord(String stringToSearch, String hiddenWord) {
		if(hiddenWord.length()==0)
			return true;
		if(stringToSearch.length() ==0 && hiddenWord.length()!=0)
			return false;
		if(stringToSearch.charAt(0)== hiddenWord.charAt(0))
			return hasHiddenWord(stringToSearch.substring(1),hiddenWord.substring(1));
		return hasHiddenWord(stringToSearch.substring(1),hiddenWord);
	}
	...
	public static boolean isPowerOf(int larger, int smaller) {
		...
		if(larger%smaller!=0)
			return false;
		return isPowerOf(larger/smaller,smaller);
