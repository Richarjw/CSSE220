...
public class TeamGradebook {
	...
	private String handleAddStudent(String studentName) {
		...
		// then return "ok"
		Student addStud = new Student(studentName);
		this.students.add(this.students.size(), addStud);
		return "ok";
	}
	...
	private String handleGetNames() {
		...
		{
			listNames = listNames + this.students.get(i).getName() + " ";
		}
		return listNames.trim();
	}
	...
	private String handleAddTeam(String teamName, ArrayList<String> memberNames) {
		...
		//TODO: Your code here
		ArrayList<Student> nTeam = new ArrayList<Student>();
		Student nStudent = null;
		for(String i:memberNames)
		{
			if(this.getStudentByName(i) == null)
			{
				nStudent = new Student(i);
				this.students.add(nStudent);
				nTeam.add(nStudent);
			}
			nTeam.add(this.getStudentByName(i));
		}
		Team teamCreate = new Team(teamName, nTeam);
		this.teams.add(teamCreate);
		return "ok";
	}
	...
	private String handleAddGrade(String teamName, double grade) {
		//TODO: Your code here
		Team teamGraded = this.teams.get(0);
		for(int i = 0; i < this.teams.size(); i++)
		{
			if(this.teams.get(i).getName().equals(teamName))
			{
				teamGraded = this.teams.get(i);
			}
		}
		teamGraded.addGrade(grade);
		return "ok";
	}
	...
	private String handleGetAverage(String studentName) {
		//TODO: Your code here
		Student stud = getStudentByName(studentName);
		if(stud.getGrades().size() == 0)
		{
			return "0";
		}
		double sum = 0.0;
		for(Double i: stud.getGrades())
		{
			double toAdd = i;
			sum = sum + toAdd;
		}
		Long average = Math.round(sum/stud.getGrades().size());
	...
	private String handleGetBestTeam() {
		//TODO: Your code here
		Team bestTeam = this.teams.get(0);
		double bestAverage = 0.0;
		for(int i = 0; i<this.teams.size(); i++)
		{
			if(this.teams.get(i).getTeamAverage() > bestAverage)
			{
				bestTeam = this.teams.get(i);
				bestAverage = this.teams.get(i).getTeamAverage();
			}
		}
